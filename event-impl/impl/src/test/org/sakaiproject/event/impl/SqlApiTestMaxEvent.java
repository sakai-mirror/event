...
public class SqlApiTestMaxEvent extends AbstractSqlApiTest {
	 ...
	 */
	public SqlApiTestMaxEvent(String name) throws SQLException, Exception {
	...
	public void testCheckDataLoaded() throws Exception {
		...
		int rowCount = loadedDataSet.getTable("SAKAI_EVENT").getRowCount();
	...
	public void testSelectMaxEventId() throws Exception {
		...
		queryDataSet.addTable("SAKAI_EVENT",maxEventIdSql);
		queryDataSet.getTable("SAKAI_EVENT");
		ITable f = fxdf.getTable("SAKAI_EVENT");
		Integer f_maxId = new Integer(((String)f.getValue(0,"EVENT_ID")));
		ITable q = queryDataSet.getTable("SAKAI_EVENT");
		Integer q_maxId = (Integer) q.getValue(0,"");
	
		assertEquals("wrong max id",f_maxId,q_maxId);
